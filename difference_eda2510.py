...
# ---------------------------------------------------------------------
OLDER_FILE = "5.0 FY2025 Material Control.csv"  # "as of Aug"
NEWER_FILE = "7.0 FY2025 Material Control.csv"  # "as of Sep"
...
def extract_material_row(df, material_name):
    ...
    if not mask.any():
        ...
        raise ValueError(f"Material '{material_name}' not found. Available: {available_materials[:10]}")
    idx = mask.idxmax()
    return df.iloc[idx]
...
# -> returns DataFrame with Month, Plan_Aug, Plan_Sep
# ---------------------------------------------------------------------
def build_metric_df(df_old, df_new, month_col, value_col,
                    label_old="Plan_Aug", label_new="Plan_Sep"):
...
def preprocess(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Aug"] = pd.to_numeric(df["Plan_Aug"], errors="coerce")
    df["Sep"] = pd.to_numeric(df["Plan_Sep"], errors="coerce")

    df["AbsDiff"] = df["Sep"] - df["Aug"]

    denom = df["Aug"].abs().replace(0, np.nan)
    df["PctDiff"] = (df["Sep"] - df["Aug"]) / denom * 100.0
...
def plot_dashboard(df: pd.DataFrame, label: str, out_prefix: str):
    ...
    fig.suptitle(f"{label} Plan Change: Sep vs Aug", fontsize=14, fontweight="bold")
    ...
    ax_hist.set_title("Distribution of Monthly Change (Sep - Aug)")
    ...
    ax_hm.set_title("Heatmap of % Change vs Aug")
...
def main():
    ...
    try:
        ...
        cons_df = pd.DataFrame({
            'Month': df_old.index,
            'Plan_Aug': df_old['consumption_planned'],
            'Plan_Sep': df_new['consumption_planned']
        }).dropna()
        
        purch_df = pd.DataFrame({
            'Month': df_old.index,
            'Plan_Aug': df_old['purchase_planned'], 
            'Plan_Sep': df_new['purchase_planned']
        }).dropna()

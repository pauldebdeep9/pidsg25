...
plot_price_distribution_band(price_df_s1, price_df_s2_flat, order_placed=order_placed, start_date=START_DATE)
...
try:
    ...
    # Plot 1: Combined Orders Comparison - Without Storage Constraint
    fig, ax1 = plt.subplots(figsize=(16, 8))
    
    # Create date index for all periods
    order_date_index = pd.date_range(START_DATE, periods=T, freq="MS")
    
    # Two bars side-by-side for Step 1 only
    bar_width = 8  # days
    offset = pd.Timedelta(days=bar_width * 0.6)
    
    ax1.bar(order_date_index - offset, order_supplier_1, width=bar_width, alpha=0.7, 
            label="Unhedged (No Storage Limit)", color='tab:blue', edgecolor='black', linewidth=0.5)
    ax1.bar(order_date_index + offset, order_supplier_2, width=bar_width, alpha=0.7, 
            label="Hedged (No Storage Limit)", color='tab:orange', edgecolor='black', linewidth=0.5)
    ...
    # Plot mean prices for both suppliers
    ...
    plt.title("Orders and Prices: Without Storage Constraint", fontsize=14)
    plt.tight_layout()
    plt.show()
    
    # Plot 2: Orders Comparison - With Storage Constraint
    fig, ax1 = plt.subplots(figsize=(16, 8))
    
    ax1.bar(order_date_index - offset, order_supplier_1_opt, width=bar_width, alpha=0.7, 
            label="Unhedged (With Storage Limit)", color='lightblue', edgecolor='black', linewidth=0.5)
    ax1.bar(order_date_index + offset, order_supplier_2_opt, width=bar_width, alpha=0.7, 
            label="Hedged (With Storage Limit)", color='lightsalmon', edgecolor='black', linewidth=0.5)
    ...
    plt.title("Orders and Prices: With Storage Constraint (≤18000)", fontsize=14)
    plt.tight_layout()
    plt.show()
    
    # Inventory/Storage comparison
    ...
    # Cost breakdown comparison - grouped bars with percentage difference
